...
默认会对中文名进行转义，导致`git status`命令显示乱码。
...
Windwos系统需要设置，因为服务器上的代码版本通常是LF，本地的是CRLF。详情见[[git-crlf]]
...
也可以直接用`git commit --amend --reset-author`来使用当前配置的作者信息来重新提交。
